...
7. Tag-sensitive text
	...
	b. To accomplish this, it is possible to enclose a portion of the text along with the name, and provide 
		...
		This is done by appending a double colon "::" to the property name, inside the brackets [], followed by variants 
...
14. Grammar and Scripted Localization
    a. It is possible to use scripted localization in place of proper in-game Names. To do so, the localization_keys
		referenced by the script are tagged as per 4. and 5. above. 
	b. When referencing such scripted loc in a localization string, you *must* either use context tags (8. above)
		OR dependent strings (7. above). If you use neither, the result will simply be the raw string
		including all variants and control characters.
	c. Example:
		 "[GetStartingDrivePlural]" will not work, but
		 "[GetStartingDrivePlural&genitive]" will.
	d. If you want to use the default variant (which likely will have no variant tag), you may use any string
	    that doesn't match a variant, e.g. "[GetStartingDrivePlural&nominative]" or "[GetStartingDrivePlural&x]"
	e. Any loc strings used in scripted localization that are not amended with variants or tags will work as normal;
		they just will not provide any grammatical functionality.